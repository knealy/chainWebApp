...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateApiDropdown() {
        try {
            console.log('Fetching API connections...');
            const response = await fetch('/api-connections');
            const data = await response.json();
            console.log('API connections data:', data);

            const apiSelectElement = document.getElementById('api-select');
            if (!apiSelectElement) return;

            apiSelectElement.innerHTML = '<option value="">Select an API</option>';
            if (data.success && data.connections) {
                data.connections.forEach(connection => {
                    const option = document.createElement('option');
                    option.value = connection.name;  // Use name as value
                    option.textContent = connection.name;
                    apiSelectElement.appendChild(option);
                });
            }
        } catch (error) {
    ...
    // Fetch and populate triggers and actions based on selected API
    async function updateEventActionDropdowns(apiId) {
        try {
            const response = await fetch(`/api-events/${encodeURIComponent(apiName)}`);
            const data = await response.json();
            ...
            if (data.success) {
                data.events.forEach(event => {
                    const option = document.createElement('option');
                    option.value = event.name;  // Use name as value
                ...
                data.actions.forEach(action => {
                    const option = document.createElement('option');
                    option.value = action.name;  // Use name as value
    ...
    // Event listener for API selection change
    const apiSelectElement = document.getElementById('api-select');
    if (apiSelectElement) {
        apiSelectElement.addEventListener('change', (e) => {
            const selectedApi = e.target.value;
            if (selectedApi) {
                updateEventActionDropdowns(selectedApi);
            } else {
                // Reset triggers and actions if no API is selected
                document.getElementById('event-select').innerHTML = '<option value="">Select a Trigger</option>';
                document.getElementById('action-select').innerHTML = '<option value="">Select an Action</option>';
            }
        });
    }
    ...
    document.getElementById('createWorkflowButton').addEventListener('click', () => {
        ...
        try {
            // Use event name as triggerName and action name as actionName
            const triggerName = eventId;  // Assuming eventId is the event name
            const actionName = actionId;  // Assuming actionId is the action name
            ...
            functionManager.createDynamicFunction(
                `workflow-${apiId}-${triggerName}-${actionName}`,
                triggerName,
                conditionFunction,
                actionName
            );
